...
class Schemas:
    """The Schema Converter object that implement converter methods of schema config data
    like `data_type`, `rename_cols`, etc.
    ...
    mapping_necessary_keys: dict = {
        'alias': {'name', 'datatype', 'type', },
        'nullable': {'na', 'null', },
        'pk': {'primary_key', 'primary', },
        'unique': {'uq', },
        'default': {'df', 'fill_na', },
        'fk': {'foreign_key', 'foreign', }
    }

    def __init__(self, schemas: dict):
        self._schemas: dict = {col: self.prepare(schemas[col]) for col in schemas}
    ...
    def prepare(self, value: Union[str, dict]) -> dict:
        """Return mapping of column properties which included `cls.mapping_necessary_keys`."""
        if not isinstance(value, (dict, str)):
            raise AttributeError(
                f'value of schema mapping data does not support for type: {type(value)}.'
            )
    ...
    def data_type(self, style: str = 'new') -> dict:
        """Return the mapping of data type that can switch the column name by old or new style
        from the configuration data like,
        ...
        """
        assert style in {'old', 'new'}, "the `style` should equal the only one of 'old' or 'new' value"
        _result: dict = {}
        for col, mapping in self._schemas.items():
            _col_old, _type = rsplit_default(mapping['alias'], '::', maxsplit=1)
...
class Statement:
    ...
    :usage:
        ...
        ...     "select {columns} from {{database}}.{schema}.{table} where {table}={condition};"
        ... )
    ...
    def load(cls, stm: Union[str, dict, List[str]]):
        """Dialect load value with not string type of value in mapping statement."""
        ...
        for _ in stm:
            if not isinstance((_value := stm[_]), str):
                raise NotImplementedError(
                    f'{self.__class__.__name__} dose not support for not string '
                    f'type in value of mapping statement.'
                )
    ...
    def parameters(self) -> list:
        ...
        return sorted(
            frozenset(re.findall(r'{(\w.*?)}', re.sub(r'({{|}})', '', self._st_stm)))
        )
...
class CronPart:
    ...
    def __init__(
        ...
        if isinstance(values, str):
            values: List[int] = self.from_string(values) if values != '?' else []
        ...
        unique_values: List[int] = self.out_of_range(sorted(dict.fromkeys(values)))
    ...
    def __repr__(self):
        return f'<{self.__class__.__name__}(unit={self.unit}, values={self.to_string()!r})>'
    ...
    def step(self) -> Optional[int]:
        ...
        if len(self.values) > 2 and (step := self.values[1] - self.values[0]) > 1:
            return step
    ...
    def from_string(self, value: str) -> List[int]:
        """Parses a string as a range of positive integers. The string should include only
        `-` and `,` special strings.

        TODO: support for `L`, `W`, and `#`
        TODO:     if you didn't care what day of the week the 7th was, you could enter ? in the Day-of-week field.
        TODO: L : the Day-of-month or Day-of-week fields specifies the last day of the month or week.
            DEV: use -1 for represent with L
        TODO: W : In the Day-of-month field, 3W specifies the weekday closest to the third day of the month.
        ...
        for _value in self.replace_alternative(value.upper()).split(','):
            if _value == '?':
                ...
                raise ValueError(f'Invalid value {_value!r} in cron part {value!r}')

            value_range, value_step = split_default(_value, '/', maxsplit=1)
            value_range_list: List[int] = self.out_of_range(self._parse_range(value_range))

            if (value_step and not check_int(value_step)) or value_step == '':
                raise ValueError(
    ...
    def replace_weekday(self, values: Union[List[int], Iterator[int]]) -> List[int]:
    ...
    def out_of_range(self, values: List[int]) -> List[int]:
        ...
        if values:
            if (first := values[0]) < self.unit['min']:
                raise ValueError(f'Value {first!r} out of range for {self.unit["name"]!r}')
            elif (last := values[-1]) > self.unit['max']:
                raise ValueError(f'Value {last!r} out of range for {self.unit["name"]!r}')
        return values
    ...
    def _interval(self, values: List[int], step: Optional[int] = None) -> List[int]:
        """Applies an interval step to a collection of values."""
        if not step:
            return values
        elif (_step := int(step)) < 1:
            raise ValueError(f'Invalid interval step value {_step!r} for {self.unit["name"]!r}')
    ...
    def is_full_interval(self) -> bool:
        ...
        if step := self.step:
            return (
                    self.min == self.unit['min'] and
                    (self.max + step) > self.unit['max'] and
    ...
    def to_string(self) -> str:
        ...
        if self.is_interval:
            ...
            _hash: str = (
                ...
                if _hash == 'H' else f'{self.filler(self.min)}-{self.filler(self.max)}'
            )
            return f'{_hash}/{self.step}'
    ...
    def filler(self, value: int) -> Union[int, str]:
        ...
        return (
            ...
            if (
                    (self.options['output_weekday_names'] and self.unit['name'] == 'weekday') or
                    (self.options['output_month_names'] and self.unit['name'] == 'month')
            )
            else value
        )
...
class CronJob:
    ...
    def __init__(
        ...
        self._options: Dict[str, bool] = merge_dict(self.options_defaults, (option or {}))
...
class CronRunner:
    ...
    __slots__ = ('tz_info', 'date', 'start_time', 'cron', 'reset_flag', )
    ...
    def next(self) -> datetime:
        ...
        self.date = self.date if self.reset_flag else (self.date + timedelta(minutes=+1))
    ...
    def _shift_date(
        ...
        switch: dict = {
            'month': 'year', 'day': 'month', 'hour': 'day', 'minute': 'hour',
        }
        current_value: int = getattr(self.date, switch[mode])
        _addition: callable = (
            (lambda: WEEKDAYS.get(self.date.strftime("%a")) not in self.cron.dow.values)
            if mode == 'day' else lambda: False
        )
        while (
                (getattr(self.date, mode) not in getattr(self.cron, mode).values) or
                _addition()
        ):
            self.date: datetime = next_date(self.date, mode=mode, reverse=reverse)
            self.date: datetime = replace_date(self.date, mode=mode, reverse=reverse)
...
def test_cron():
    ...
    print(cr1 == cr2)
    print(cr1 < cr2)
    cr = CronJob('*/5,3,6 9-17/2 * 1-3 1-5', option={
        'output_hashes': True,
    })
    print(cr)
    cr = CronJob('*/5 9-17/2 * 1-3,5 1-5', option={
