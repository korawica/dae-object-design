...
from application.io import join_path
from application.core.converter import Schemas
from application.utils.type import (
    merge_dict,
)
from application.errors import CatalogArgumentError


class BaseCatalog:
    """Base Catalog object. This object will design necessary properties and
    methods for any kind of sub class catalog types.
    """
    ...
    def option(self, key, value):
        ...
        super(BaseCatalog, self).__setattr__(f"_{key}", value)
        return self
...
class PandasCSVFile(BaseCatalog):
    ...
    def props_save(self, addition: Optional[dict] = None) -> dict:
        ...
        """
        _props: dict = merge_dict(self.properties, self._props_save, (addition or {}))

        # Change value from main properties to save properties.
        _header: bool = (_props.pop('header', True)) is not None
        _delimiter: str = _props.pop('delimiter', ',')
        _mode: str = 'w' if (_m := _props.pop('mode', 'overwrite')) == 'overwrite' else _m[0]
    ...
    def load(
        ...
        if _props.get('typ') == 'series':
            ...
            if _props.get('orient') == 'records':
                ...
                _df: pd.DataFrame = series.to_frame(name='value').reset_index(names='key')
...
class PandasSQLite(BaseCatalog):
    ...
    def properties(self) -> dict:
        return merge_dict({
            'columns': list(self._schemas.data_type(style='new'))
        }, self._props)
    ...
    def load(
        ...
        _stm: str = f"select * from {self._endpoint} limit {limit}" if limit else self._endpoint
        return pd.read_sql(
            _stm,
            con=conn.connection,
            **self.properties
    ...
    def save(self, df: Any, conn: Optional = None, *, option: Optional = None) -> None:
        ...
...
def test_pandas_local():
    ...
    with Connection('demo:conn_local_file').connect() as conn:
        df = PandasCSVFile('demo/landing/customer.csv').load(conn)
        print(df)
...
def test_pandas_abfs():
    ...
    with Connection('demo:conn_azure_blob_de_test').connect() as conn:
        df = PandasJsonFile('scg_cbm_do_dev_rg_bf435d1a3dc9-718bf.json', properties={
            'orient': 'columns',
            'typ': 'series',
        }).load(conn)
        print(df)
