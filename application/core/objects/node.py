...
from application.errors import NodeArgumentError
from application.core.objects.action import BaseAction
...
class PandasAction(BaseAction):
    ...
    class Distinct(BaseAction.Distinct):
        ...
        def action(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
            return df.drop_duplicates(subset=self.props['columns'], keep=self.props['keep'])
    ...
    class OrderBy(BaseAction.OrderBy):
        ...
        def action(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
            ...
            return df.sort_values(list(_cols.keys()), ascending=list(_cols.values()))
        ...
        def _aggregate(self):
            ...
            for k in _aggs:
                if isinstance((getter := _aggs[k]), str):
                    ...
                    _aggs[k] = (_new[0], eval(_new[1])) if _new[1].startswith('lambda') else _new
                elif isinstance(getter, list):
                    # Convert the list value of column and func to tuple
                    _func = eval(getter[1]) if getter[1].startswith('lambda') else getter[1]
    ...
    class Join(BaseAction.Join):
        ...
        def action(
            ...
            return df.join(
                other, on=self.props['on'], how=self.props['how'], validate=self.props['validate']
            )

    class Union(BaseAction.Union):
        others: List[pd.DataFrame]

        def action(self, df: pd.DataFrame, others: Optional[List[pd.DataFrame]] = None) -> pd.DataFrame:
            if others is None:
    ...
    class DataQuality(BaseAction.DataQuality):
        ...
        def action(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
            if (_dqf := getattr(self, f"_dq_{self.props['dq_function']}", None)) is None:
                raise NodeArgumentError(
                    'dq_function', f'the data quality function {self.props["dq_function"]!r} '
        ...
        def _dq_outlier(self, df: pd.DataFrame) -> pd.DataFrame:
            ...
            for col in self.props['columns']:
                df[f"{col}_dq_outlier"] = (np.abs(stats.zscore(df[col])) >= _std_value)
            return df
...
class BaseNode:
    ...
    def get_input(self, name: str) -> pd.DataFrame:
        ...
        if name in self._transform_result:
            ...
            return self._input[name]['data'].load(option=self._input[name]['params'])
        raise NodeArgumentError(
            ('input', 'from', ),
    ...
    def runner(
        ...
        for task in self.transform:
            if actions := task.get('actions'):
                print(f'This task: {task["alias"]!r} will running in action mode ...')
    ...
    def run_action(
        ...
        for action in actions:
            ...
            if action.get('type').lower() == 'join':
                ...
                _others: List[pd.DataFrame] = [self.get_input(other) for other in action.pop('others')]
                action['others'] = _others
            _input: pd.DataFrame = self.base_action.from_data(action).action(_input)
