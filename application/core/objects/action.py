...
from ...utils.type import isinstance_check
...
class BaseActionType:
    """Base Action Type object for parsing parameters from configuration data to any
    action sub-class object
    """
        ...
        try:
            for k, _typing in self.__annotations__.items():
                ...
                if not isinstance_check(value, _typing):
                    raise TypeError(
                        f'type of {k!r} for {self.__class__.__name__} does not match '
                        f'with {_typing}'
                    )
                self._props[k] = value
...
class BaseAction:
    ...
    def __repr__(self):
        _props: str = f", properties={self._act_props}" if self._act_props else ""
        return f"<{self.__class__.__name__}(action_type={self._act_type}{_props})>"
        ...
        try:
            ...
            for _arg in _args[2:]:
                if _arg in kwargs:
                    ...
                    raise NodeArgumentError(
                        _arg, f'type: {_inspect_f.annotations[_arg]} does not exists '
